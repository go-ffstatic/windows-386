...
func init() {
	var err error
	ffmpegPath, err = writeTempExec("ffmpeg_windows_386", ffmpeg)
	if err != nil {
		panic(fmt.Errorf("failed to write ffmpeg_windows_386: %v", err))
	}
	ffprobePath, err = writeTempExec("ffprobe_windows_386", ffprobe)
	if err != nil {
		panic(fmt.Errorf("failed to write ffprobe_windows_386: %v", err))
	}
}
